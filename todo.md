...
- [x] 盤點完整搬遷所需的程式碼、服務、設定與資料
- [x] 撰寫新設備重建、驗證與切換舊設備的完整流程

## 語音品質與速度優化測試

- [x] 盤點 Voicebox 的文字、速度、情緒與克隆品質可調整項目
- [x] 建立可重複執行的測試語料、評分表與優化建議
...
- [x] 定位品質修正後的 HTTP 504 為同步長連線遭代理層關閉
- [x] 改為後端非同步生成任務與 APP 短連線輪詢，排除同步等待 504
- [x] 修正 Metro 提早結束時連帶終止 API 與背景任務的開發服務設定
- [ ] 以實機完成一筆非同步生成任務並確認不再顯示 HTTP 504
- [x] 將完成任務輪詢延長為 30 分鐘，確保 APP 會自動取回較慢的最終音檔
- [x] 確認 Voicebox captures 設定已使用 Whisper Large
...
- [x] 以日日誦經、祝禱加持、蔡承諺指定注音文字端到端測試，確認 instruct 規則仍未能約束 Qwen G2P
- [x] 實測並自動套用 Qwen 同音常用字代理格式，修正已知罕見字與人名讀音
- [x] 檢查搬遷後同音代理與手動注音覆寫規則仍可正確套用
- [x] 將情緒選擇改為單一主情緒，並保留自訂提示加強語氣
...
- [ ] 比對新舊最佳與最新 Profile 的樣本、模型、參考文字和生成指令，恢復聲音品質
- [ ] 安全還原舊電腦最佳 Voicebox Profile 資料並以固定測試文字驗證音色

## 多家屬聲音身份與品質保證

- [x] 設計一位親友一個受管控 Voicebox Profile 的聲音檔案庫流程
- [x] 定義參考音檔、逐字參考文字、預覽評分與正式啟用的品質驗收門檻
- [x] 規劃家屬資料隔離、既有 Profile 重用與新音檔增補的安全規則

## 首次高品質語音克隆強化

- [x] 定義高相似度參考音檔與逐字參考文字的最低品質標準
- [x] 設定首次生成的穩定模型、情緒、語速與讀音預設值
- [x] 規劃自動音檔品質分析、預覽與不合格樣本攔截機制

## 授權語音測試與家屬錄音指南

- [x] 整理公開授權測試素材的克隆品質與穩定度評估方法
- [x] 制定系統端音檔預處理、品質評分與拒絕門檻
- [x] 撰寫可提供家屬的高品質聲音範本錄音黃金守則

## 替代開源語音克隆引擎評估

- [ ] 比較可本機部署且支援中文的免費開源語音克隆模型、授權與硬體需求
- [ ] 定義與現有 Voicebox 相同輸入樣本與測試文字的品質比較方法
- [ ] 提出優先試跑引擎與逐步整合方案，不影響既有 Voicebox 服務

## Voicebox 優先品質優化

- [ ] 盤點現有 Voicebox Profile、參考文字與生成設定，建立可比較的品質基準
- [x] 實作已核可 Voicebox Profile 重用，避免每次上傳建立新聲音身份
- [x] 實作候選 Profile 版本保護，避免新樣本覆蓋既有最佳聲音
- [x] 實作音檔品質閘門與單一說話者確認流程（時長、有效語音、音量、clipping）
- [x] 建立三段預覽、家屬核可與正式 Profile 啟用流程

## 自建本機語音克隆服務可行性評估

- [x] 盤點目前 APP、API、中介服務、Tunnel 與 Voicebox 之間的延遲及檔案傳輸瓶頸
- [x] 研究可重用開源模型的自建語音克隆服務組成、硬體需求與授權限制
- [x] 設計不影響正式 APP 的本機旁路測試架構與逐步遷移方案

## GitHub 同步核對

- [x] 核對本機 Git 狀態、提交紀錄與 GitHub `main` 分支是否一致
- [x] 如有未同步內容，提交並推送至 GitHub 遠端儲存庫

## 專案架構與交接文件

- [x] 整理 EchoAPP 架構總覽、核心資料流與重要檔案索引
- [x] 撰寫新 AI／開發者交接指南，列出 Voicebox 已完成優化與後續待辦
- [x] 驗證文件後提交並推送至 GitHub `main`

## 聲音身份開發者選項

- [x] 將 Voicebox 聲音身份管理與候選核可介面從首頁移至設定頁
- [x] 建立密碼保護的開發者選項，鎖定聲音身份管理功能
- [x] 簡化首頁為家屬使用的聲音選擇與生成流程
- [x] 驗證開發者鎖定、聲音重用與既有生成流程不受影響

## 圖示更新與首頁上傳復原

- [x] 生成並套用與入場動畫一致的迴響音波 App 圖示至所有啟動與應用圖示資產
- [x] 恢復首頁授權音檔選擇與一次性語音生成流程，不顯示身份管理控制
- [x] 清楚說明首頁正式聲音重用與本次上傳音檔的使用優先順序
- [x] 驗證圖示、首頁上傳與既有開發者身份管理流程

## 家屬音檔預聽與影片支援

- [x] 恢復首頁上傳音檔的試聽播放、停止與播放狀態回饋
- [x] 恢復家屬填寫音檔聲音特色、情境與注意讀音的描述欄位
- [x] 支援選取常見手機影片（MP4、MOV）並於伺服器端安全擷取音軌
- [x] 將影片音軌擷取後的品質檢查與 Voicebox 上傳流程串接
- [x] 驗證音訊、影片、預聽與既有正式聲音重用流程

## 使用者指定 App ICON

- [x] 將使用者提供的迴響 ICON 轉換為 Expo 相容 PNG 圖示資產
- [x] 取代應用、啟動畫面、網站與 Android 自適應圖示的既有資產
- [x] 驗證 app.config.ts 品牌設定、圖示尺寸與專案型別檢查

## 家屬短音檔與設定選項修正

- [x] 將少於 20 秒的授權參考音檔改為品質提醒而非拒絕生成
- [x] 統整首頁為單一媒體檔案選擇按鈕，支援音檔與影片
- [x] 補回語氣與個性設定中遺漏的兩個選項
- [x] 驗證短音檔、單一選檔入口與個性提示可正常送往 Voicebox

## 首頁載入與關於迴響文案修正

- [x] 修正首頁正式聲音讀取逾時時持續顯示「確認聲音設定中」的問題
- [x] 確保選擇本次授權素材時不受正式聲音載入狀態阻擋
- [x] 將設定頁關於迴響恢復為家屬導向說明並以 AI 語音克隆描述技術
- [x] 移除設定頁面向家屬內容中的特定服務名稱並驗證流程

## 語音亂語回歸與儲存按鈕修正

- [x] 比對穩定版本與目前參考文字、轉錄、短素材及生成指令資料流
- [x] 修復導致 AI 語音亂語的參考文字與短素材防護回歸
- [x] 移除儲存音檔區的無效空白按鈕
- [x] 驗證讀音詞庫、生成資料流與儲存操作均正常

## 指定詞語讀音修正

- [x] 檢查畢業專題、軟體、緬懷、療癒在現有讀音代理規則中的輸入與輸出
- [x] 加入安全的詞語讀音代理與手動注音覆寫相容規則
- [x] 為四個指定詞語與原始正字顯示加入回歸測試

## 指定詞語深化與深色模式修正

- [x] 調整畢業專題、軟體、療癒、優化的合成端讀音代理與精確提示
- [x] 修復深色模式下編輯回憶白色按鈕的文字對比
- [x] 核對使用者指定 ICON 的所有 Expo 資產與實機快取更新方式
- [x] 驗證讀音規則、深色模式按鈕與圖示設定

## 常見詞讀音回歸修正

- [x] 檢查畢業與供用戶製作在合成端代理及讀音提示中的處理結果
- [x] 加入畢業與供用戶製作的整詞讀音修正規則
- [x] 擴充常見詞文字轉檔回歸測試並驗證原始正字顯示
